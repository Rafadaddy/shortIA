...
import { Sparkles, Copy, Check, Image as ImageIcon, Loader2, RefreshCw, Wand2, Type, Home } from "lucide-react";
...
const mexTopics = [
  "ECONOMÍA (La tanda, Coppel, el recibo CFE)",
  "LA FAMILIA EXTENSA (Suegra, arrimados, el terreno)",
  "DRAMA Y CONFLICTOS (La chancla, la tele, chismes)",
  "FIESTAS Y COMIDA (Tamales, 15 años endeudados)",
  "PROBLEMAS FÍSICOS DE CASA (El boiler, tinaco, goteras)",
...
const mexProtagonists = [
  "Mamá luchona (con la chancla en mano)",
  "Papá proveedor (el que apaga todas las luces)",
  "Abuela sabia (la dueña de los terrenos)",
...
const mexTones = [
  ...
  "Terror de mamá enojada",
  "Motivacional (Echándole ganas)"
];

const mexStyles = [
  "POV TikTok Casero (Cámara celular realista)",
  "Casa de Infonavit (Luz de foco ahorrador)",
  "Cinematográfico Exagerado (La Rosa de Guadalupe)",
  "Estética Vintage Mexicana (Foto 90s)"
];
...
export default function CasasMexicanasPage() {
  ...
  const generateIdeas = async () => {
    ...
    setData(null);
    try {
      const res = await aiFetch("/api/generate-mexico", { mode: "ideas", topic, tone, protagonist });
  ...
  const selectIdeaAndGenerateScript = async (idea: MexIdea) => {
    ...
    setData(null);
    try {
      const res = await aiFetch("/api/generate-mexico", { mode: "script_only", idea: idea.hook, tone, protagonist, topic, sceneCount, duration });
      if (!res.ok) throw new Error("Error");
      const json = await res.json();
  ...
  const improveScript = async (instruction: string) => {
    ...
    setIsGeneratingScript(true);
    try {
      const res = await aiFetch("/api/generate-mexico", { mode: "improve_script", current_script: scriptText, instruction, tone });
      if (!res.ok) throw new Error("Error");
      const json = await res.json();
      setScriptText(json.script);
    } catch (error) {
  ...
  const generateFullVideo = async () => {
    ...
    try {
      const res = await aiFetch("/api/generate-mexico", { mode: "full_from_script", current_script: scriptText, visualStyle, sceneCount, duration });
      if (!res.ok) throw new Error("Error");
      const json = await res.json();
      setData(json);
    } catch (error) {
  ...
  const handleRegeneratePrompt = async (sceneIndex: number, promptType: "image" | "animation") => {
    ...
    try {
      ...
      const res = await aiFetch("/api/generate-mexico", {
        mode: "single_prompt",
  ...
  return (
    <main className="min-h-[calc(100vh-4rem)] p-4 md:p-6 lg:p-12 selection:bg-orange-500/30">
      <div className="max-w-5xl mx-auto space-y-8 md:space-y-12">
        ...
        <header className="text-center space-y-4">
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight text-white flex items-center justify-center gap-4">
            <Home className="w-8 h-8 md:w-10 md:h-10 text-orange-500" />
            Casas Mexicanas
          </h1>
        ...
        <div className="bg-slate-900/50 p-5 md:p-8 rounded-3xl border border-slate-800/60 shadow-2xl backdrop-blur-xl space-y-6">
          <div className="flex items-center gap-2 border-b border-slate-800 pb-4 mb-4">
            <div className="bg-orange-500/20 text-orange-400 w-8 h-8 flex items-center justify-center rounded-full font-bold">1</div>
            <h2 className="text-xl font-bold text-white">Configuración del Guion</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <select value={protagonist} onChange={(e) => setProtagonist(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-slate-200 focus:border-orange-500 outline-none">
            ...
            <div className="space-y-2">
              ...
              <select value={tone} onChange={(e) => setTone(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-slate-200 focus:border-orange-500 outline-none">
                {mexTones.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-300">Estética Visual</label>
              <select value={visualStyle} onChange={(e) => setVisualStyle(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-slate-200 focus:border-orange-500 outline-none">
            ...
            <div className="space-y-2 lg:col-span-1">
              ...
              <div className="flex items-center gap-4 bg-slate-950 border border-slate-800 rounded-xl p-3">
                <input type="range" min="3" max="15" value={sceneCount} onChange={(e) => setSceneCount(parseInt(e.target.value))} className="w-full accent-orange-500" />
                <span className="text-orange-400 font-bold min-w-[2ch]">{sceneCount}</span>
              </div>
            </div>
          </div>
          ...
          {ideas && (
            <div className="mt-8 space-y-4 animate-in fade-in">
              <h3 className="text-lg font-semibold text-orange-300 mb-2">Selecciona la situación:</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {ideas.map((idea, idx) => (
                  <button key={idx} onClick={() => selectIdeaAndGenerateScript(idea)} disabled={isGeneratingScript} className="text-left bg-slate-950 border border-slate-800 p-4 rounded-xl hover:border-orange-500/50 hover:bg-slate-900 transition group disabled:opacity-50">
                    <p className="font-bold text-orange-400 mb-1">{idea.title}</p>
        ...
        {(isGeneratingScript || scriptText) && (
          <div className="bg-slate-900/50 p-5 md:p-8 rounded-3xl border border-slate-800/60 shadow-2xl backdrop-blur-xl space-y-6 animate-in slide-in-from-bottom-4">
            <div className="flex items-center gap-2 border-b border-slate-800 pb-4 mb-4">
              <div className="bg-orange-500/20 text-orange-400 w-8 h-8 flex items-center justify-center rounded-full font-bold">2</div>
              <h2 className="text-xl font-bold text-white">Edición del Guion Mexicano</h2>
            </div>
            ...
            {isGeneratingScript && !scriptText ? (
              ...
              <div className="space-y-4">
                <textarea value={scriptText} onChange={(e) => setScriptText(e.target.value)} className="w-full h-64 bg-slate-950 border border-slate-800 rounded-xl p-4 text-slate-200 focus:border-orange-500 outline-none resize-none leading-relaxed" />
                <div className="flex flex-wrap gap-3">
                  <button onClick={() => improveScript("Hazlo más exagerado y chistoso, metiendo más palabras mexicanas como güey, nmms, chale.")} disabled={isGeneratingScript} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 py-2 rounded-lg flex justify-center items-center gap-2 transition-colors"><Type className="w-4 h-4" /> Más Mexicano</button>
                  <button onClick={() => improveScript("Haz que termine en un grito dramático o regaño de la mamá (la chancla).")} disabled={isGeneratingScript} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 py-2 rounded-lg flex justify-center items-center gap-2 transition-colors"><RefreshCw className="w-4 h-4" /> Final de Chancla</button>
                </div>
              </div>
            )}
          </div>
        ...
        {scriptText && !isGeneratingScript && (
          <div className="bg-slate-900/50 p-5 md:p-8 rounded-3xl border border-slate-800/60 shadow-2xl backdrop-blur-xl space-y-6 animate-in slide-in-from-bottom-4">
            <div className="flex items-center gap-2 border-b border-slate-800 pb-4 mb-4">
              <div className="bg-orange-500/20 text-orange-400 w-8 h-8 flex items-center justify-center rounded-full font-bold">3</div>
              <h2 className="text-xl font-bold text-white">Generar Escenas Visuales</h2>
            </div>
            <button onClick={generateFullVideo} disabled={isGeneratingVideo} className="w-full py-4 rounded-xl font-bold bg-orange-600 hover:bg-orange-500 text-white flex items-center justify-center gap-2 transition-all">
            ...
            <div className="bg-slate-900/50 p-5 md:p-8 rounded-3xl border border-slate-800/60 shadow-2xl">
              <div className="flex justify-between items-center mb-8 border-b border-slate-800/60 pb-6">
                <div>
                  ...
                  <p className="text-orange-400 text-sm font-semibold">Guion listo para grabar</p>
                </div>
                <button onClick={handleCopyAll} className="flex items-center gap-2 bg-orange-600/20 text-orange-400 py-2 px-4 rounded-xl text-sm font-semibold hover:bg-orange-600/40 transition-colors">
                  {copiedStates['all'] ? <><Check className="w-4 h-4" /> Copiado</> : <><Copy className="w-4 h-4" /> Copiar Todo</>}
                </button>
              </div>

              {/* Thumbnail */}
              <div className="bg-slate-950 p-6 rounded-2xl border border-orange-500/30 mb-8 relative">
                <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><ImageIcon className="w-5 h-5 text-orange-400" /> Idea de Miniatura</h3>
                <div className="space-y-4">
                  <div>
                    <span className="text-xs font-bold text-orange-400 uppercase">Texto Sugerido:</span>
                    <p className="text-white font-bold text-lg mt-1">{data.thumbnail.text}</p>
                  </div>
                  <div>
                    <span className="text-xs font-bold text-orange-400 uppercase">Prompt de Imagen:</span>
              ...
              <div className="space-y-6">
                {data.scenes.map((scene, idx) => (
                  <div key={scene.scene_number} className="bg-slate-950 p-5 rounded-2xl border border-slate-800 relative group overflow-hidden">
                    <div className="absolute top-0 left-0 w-1 h-full bg-slate-700 group-hover:bg-orange-500/50 transition-colors"></div>
