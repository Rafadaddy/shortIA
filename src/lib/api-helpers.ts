...
export async function chatCompletion(
  ...
  // PRIORIDAD 1: Usar la API Key y configuraci√≥n enviada desde el cliente (UI)
  if (providerConfig && providerConfig.apiKey) {
    console.log(`[API-Helpers] Usando configuraci√≥n del cliente: ${providerConfig.providerId} | Modelo: ${providerConfig.model}`);
    try {
  ...
  // PRIORIDAD 2: Usar NVIDIA si est· configurado (M·xima velocidad e inteligencia)
  if (nvidiaKey) {
    ...
    try {
      ...
      console.error(`[API-Helpers] NVIDIA fallÛ, pasando a Gemini...`);
    }
  }
  ...
  if (geminiKey) {
    ...
    for (const key of keys) {
      try {
        ...
        console.error(`[API-Helpers] La llave de Gemini terminada en ...${key.slice(-4)} fall√≥. Intentando con la siguiente si existe...`);
      }
    }
    console.error(`[API-Helpers] Todas las llaves de Gemini fallaron. Pasando al salvavidas final (Groq).`);
  }

  // PRIORIDAD 3: Usar la API Key de Groq local del servidor (process.env) como √∫ltimo fallback
  if (groqKey) {
