...
content = content.replace(
  ...
  'import { Sparkles, Copy, Check, Image as ImageIcon, Loader2, RefreshCw, Wand2, Type, Swords } from "lucide-react";'
);
...
const headerUI = `<div className="flex items-center gap-4 mb-8 animate-in fade-in slide-in-from-top-4">
...
content = content.replace(
  /<div className="flex items-center gap-4 mb-8 animate-in fade-in slide-in-from-top-4">[\s\S]*?<\/div>\s*<\/div>/,
  headerUI
);

const paso1UI = `        <div className="bg-slate-900/50 p-6 md:p-8 rounded-3xl border border-slate-800/60 shadow-2xl backdrop-blur-xl animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="space-y-2 mb-6">
              ...
              <select value={tone} onChange={e => setTone(e.target.value)} className="w-full bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-3 text-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500/50">
                ...
                <option value="Gamer / TierZoo">Gamer / RPG (Stats, DPS, Tanque)</option>
              </select>
          </div>
...
content = content.replace(
  /<div className="bg-slate-900\/50 p-6 md:p-8 rounded-3xl border border-slate-800\/60 shadow-2xl backdrop-blur-xl animate-in fade-in slide-in-from-bottom-4">[\s\S]*?<\/div>\s*<\/div>\s*<button\s*onClick=\{generateIdeas\}[\s\S]*?<\/button>\s*<\/div>/,
  paso1UI
);
